...
| ✚ `/it/aturan-gaji` → jadwal kerja | **built (M57)** — the five working patterns as a table: masuk, pulang, istirahat, Jumat, and which units are on each (Q44, D274). What a pattern does not say prints as **belum ditetapkan**, never as 00.00, and the row carries the sentence explaining it — the guard's twelve hours begin at a time nobody has fixed. A pattern with nobody on it says that too. The yearly effective-days field prints its own **monthly average** beside it, derived rather than stored (Q45, D271) | M57 |
