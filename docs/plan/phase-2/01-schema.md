...
| 0090 | `asst_turns` | **new schema `ops_asst`**; `turn_kind_t`, `refusal_t`, `assistant_turns`, trigger `turn_is_evidence`, `record_turn()`, `close_draft()`, `v_turn_provenance` | **Done.** John Lau's record, and nothing else: the dispatcher is a keyword matcher today and a language model later (D221), and the tool catalogue stays **code**, because a list of what an assistant may reach is a security boundary and a boundary stored as rows is a boundary with an UPDATE statement in it (D218). The two rules `02-database.md` asked for are constraints now — **no money in the prose**, so a figure cannot be paraphrased on its way into a sentence, and nothing validating `tools_used` against a catalogue that deliberately is not here. A turn is **append-only but for the second yes**, because a turn that produced a purchase request line is not a chat log but the **provenance of a write**, and provenance that can be tidied up afterwards is not provenance. Your own conversation and nobody else's — not even IT — while `v_turn_provenance` answers *where did this line come from* with the tool, the fields and the document and **no prompt column at all**. The only view in the ladder that runs as its owner, and the reason is in its `where` clause |
