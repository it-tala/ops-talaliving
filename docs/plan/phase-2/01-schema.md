...
| 0048 | `hr_contributions` | `contribution_scheme_t`, `contribution_rates`, `enrolments` | **Outstanding.** The statutory half, and the last HR table still blocked on an enum `0001` never carried. No enrolment, no deduction — the data is the switch (D259). Q30 gates handing a payslip to an employee, not building this |
| 00xx | `hr_tasks` | `task_status_t`, `task_ref_t`, `tasks` | **Outstanding.** The KPI board's half of HR (D260, D261), blocked on the other two enums from M50/M51 |
