...
| 0063 | `prod_vendor_legs` | `vendor_process_t`, `vendor_legs`, and the three fields above | **Outstanding.** W6 and D280. Four `subcon_*` columns could describe exactly one trip; this business sends a piece to several vendors for several processes, and a piece can visit more than one. Late is measured against a **promise**, never against silence (D134), and what came back is a quantity rather than a tick. `goodsOnSite` becomes answerable, and with it the D255 refusal |
