...
| 0046 | `hr_holiday_rule` | redefines `read_day()`, `v_leave_used`, `v_day_mark_value`; adds `office_closed()` | **Done.** The owner's answer to F101 (D283). A closed day is closed for everybody, and — the part that is not in the sentence — it spends nobody's leave entitlement. A correcting migration rather than an edit to 0045, because the ladder is the record of what the system did before somebody asked |
...
| 0050 | `hr_kpi` | `v_kpi_measure`, `v_kpi_score` | **Outstanding, and partly blocked.** Three measures — punctuality, attendance, task delivery — over the timesheet, the marks and `tasks`, with weights and floors from `ops_core.settings`. A score is withheld below `kpi.min_measures`, because one axis of three is that axis wearing a costume (D261). **The production half of the card cannot be built yet**: `personWork` and `workAttribution` read `ops_prod.progress_entries`, and `ops_prod` has no tables |
...
| 0061 | `prod_orders` | `route_t`, `process_stages` (seed), `work_orders` | **Outstanding.** The stages are **four**, not the seven Q35 assumed — D275 and D278 settled that, and they belong to the product as well as the route. Late-first ordering is a view, not a column |
| 0062 | `prod_progress` | `progress_source_t`, `progress_entries`, `v_work_order` | **Outstanding.** Append-only; a correction is a negative entry (A5); signing an overtime sheet posts progress (D147), and `0043`'s `overtime_lines.wo_no` is the seam it arrives through. **`0050_hr_kpi` is waiting on this one** — the KPI card's production half reads these entries |
| 0063 | `prod_bom_explode` | `explode_bom()` | **Outstanding.** The recursive walk (D257): waste compounds through sub-assemblies, the same material reached by two routes merges into one line with both routes named, a sub-assembly with no released BOM stays in the list as itself, and a cycle is detected rather than looped |
