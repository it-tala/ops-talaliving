...
| 0045 | `hr_timesheet` | `v_timesheet_day`, `v_payroll_line` | **Outstanding, and the largest HR item.** The six slots a day's taps are read into, the break allowance per schedule (D270, D274), the rule book's overtime tiers, and the gross figure that reads from them. ~140 lines of `timesheetDay` plus ~300 of `payrollLine` to transcribe. Approving a run is refused while `open_days > 0` (D139), and `open_days` is a timesheet figure, so that refusal belongs here too |
