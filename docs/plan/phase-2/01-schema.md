...
| 0046 | `hr_gross` | `allowance_withholdings`, `contribution_scheme_t`, `contribution_rates`, `enrolments`, `v_payroll_line` | **Outstanding.** Writing 0045 found that the gross figure needs three tables the ladder never listed: the allowance a day loses by HRD's decision (D250), and the contributions pair that is still waiting on its enum. The hourly rate, the overtime ladder, undertime and lateness are functions over the rule book and go here with the view that reads them |
