...
| Q53 | **The three gaps the working patterns still have.** The guard's twelve hours start at a time nobody has fixed, and it is not known whether the shift rotates; the house assistant's finishing time is not set; and neither pattern has a stated break | Raised 2026-09-14 (D274). None of them is guessed: a schedule with no start reads **tidak terukur** on punctuality rather than *never late*, which is the whole point. Answer any of the three and it is one line in the rule book |
| Q52 | **Does every product go through all four stages?** A plain dining table has no lamps or cables in it, so *Machinery / instalasi* is empty on almost every order | Raised 2026-09-14 (D275, F92). Handled honestly for now — an unrecorded stage is no longer read as a zero and no longer produces a *later stage overtook it* warning — but the board still draws a column that does not apply. The real answer is probably **per product**, on the BOM or the product record, and it is worth the owner saying whether a stage that does not apply should be hidden or shown empty |
| Q51 | **Is QC really not a step of its own?** The owner's four (Q47) do not include it, and unlike *Pembuatan* nothing in his answers explains why | Raised 2026-09-14 (D275). Checking before packing is either folded into Finishing, or done and not tracked, or not done — three different answers with three different consequences, and the board currently rolls old `QC` entries into Packing because that is the step they always immediately preceded. A seed edit either way |
