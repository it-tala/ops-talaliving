...
| B11 | ~~**Baris yang vendornya diputuskan di PO tidak bisa dibayar dari barisnya**~~ | `ops_acct.post_from_line` | **fixed** 2026-09-23 — `0135`: the vendor falls back to the order the line is on; a line on no order with no vendor is still refused `vendor_required` (F151) |
