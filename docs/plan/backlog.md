...
| Q45 | **What is this business's own `hari kerja efektif`?** — **reopened 2026-09-23** | The 13 September close (D271) answered *who types the figure*, not what it is, and the question was marked done anyway. 288 then went to production as if it were the owner's number; it was a six-day convention copied from the demo (F138, D288). Now that the week is confirmed as five days, **240** (20 × 12) stands in as the default — it is a convention, not an answer. The real question: 5 × 52 = 260 weekdays minus national holidays and cuti bersama, so what does this business actually count? It is not cosmetic — `hourly_rate()` divides a year's pay by it, so every overtime rupiah moves with it. **Default if unanswered**: 240 |
...
| W7 | **Halaman profil untuk setiap pengguna** — nama, email, modul dan wewenang yang dipegang (baca saja), dan tombol *ganti kata sandi* ke `/set-password` | Diminta 2026-09-23. `/set-password` sudah menangani ganti sandi saat sedang login, tetapi dari dalam aplikasi tidak ada tautan ke sana, jadi pengguna dengan sandi sementara tidak punya jalan untuk menggantinya kecuali tahu alamatnya. Profil hanya menampilkan akses, tidak mengubahnya: grant tetap di `/it/pengguna`, yang tercatat di audit |
| W8 | **Setup akun dari layar IT, bukan dari SQL** — buat akun, beri sandi sementara yang ditampilkan **sekali**, reset sandi, dan tanda *wajib ganti sandi saat login pertama* | Diminta 2026-09-23. Hari ini setiap akun baru dan setiap *lupa sandi* adalah pekerjaan SQL langsung ke `auth.users` (RUNLOG `03_chat_users`), tanpa `actor_id` di audit. Tanpa tanda wajib-ganti, sandi sementara berlaku selamanya. Butuh route server dengan service-role key (Admin API GoTrue), dijaga `it.manage_roles`. **Custom SMTP** (RUNLOG, *Why custom SMTP is still needed*) menutup separuhnya: dengan SMTP, *Lupa password* di halaman masuk bekerja sendiri dan reset tidak lagi jadi tiket IT |
