...
| B9 | **Ongkir (baris tanpa jumlah) tidak bisa dibayar dari barisnya dengan jenis SUPPLIERS** — `post_from_line` meneruskan detail tanpa jumlah, dan jenis pembelian menolaknya `line_detail_required` | `/procurement/pr` · `ops_acct.post_from_line` | open — found 2026-09-23 by the second walk (F149). Needs a rule: what a lump-sum line's detail says (D75 forbids inventing a quantity). Workaround in John Lau's FAQ |
