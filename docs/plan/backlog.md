...
| B5 | **Barang datang tidak bisa dicatat di mode live** — `ReceiveForm` mengirim jenis dokumen sebagai label (*Receiving Item*, *Delivery Note*), `create_receipt` membandingkan kode (`goods_photo`, `delivery_note`) tanpa `doc_kind_of`, jadi foto yang ada dibaca sebagai tidak ada dan penerimaan ditolak `photo_required` | `/procurement/tracker/[vendor]` · `0017` `create_receipt` | open — found 2026-09-23 by the simulation (F148). Fix is in the seam: resolve the kind through `ops_core.doc_kind_of`, as `post_transaction` does |
| B6 | **Halaman *New request* tidak live** — `/procurement/pr/new` memuat `production.listWorkOrders()` untuk pilihan *For which job in production*, dan `production` belum live, jadi seluruh halaman — satu-satunya jalan membuat PR — tertutup di sistem live | `/procurement/pr/new` | open — found 2026-09-23 (F148). The work-order picker is optional; it can be hidden when `production` is not live |
| B7 | **PO tidak tersambung ke baris PR** — `po_lines` tidak punya kolom baris PR dan `create_po` tidak menerimanya, jadi *PO dibuat dari baris yang sudah disetujui* hanya kebiasaan, bukan aturan | `ops_procure.po_lines` · `/procurement/po` | open — found 2026-09-23 (F148). A schema decision; needs the owner |
| B8 | **Termin PO tidak bisa dibayar dari layar mana pun** — `allocate_payment(p_po_no)` ada, tombolnya tidak; pembayaran masuk ke baris PR dan termin PO tetap terbaca *UNPAID* | `/procurement/po/[po]` | open — found 2026-09-23 (F148). Depends on B7 |
