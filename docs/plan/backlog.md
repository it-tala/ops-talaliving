...
| # | What | Note |
|---|---|---|
| W7 | ~~**A profile screen every account owns**~~ | **built 2026-09-24** — D305–D307, `0163`–`0166`. `/profil`: password reset (the existing road, reused as-is), a personal activity feed of the account's own safe-kind actions (D306 — deliberately not the IT trail D190 closed), self clock in/out (D307), self-service overtime and leave requests with evidence on the existing road (ADR-010), a reminder view of the account's own tasks, and a self-scoped payslip — every read composing through RLS with no second computation of the same figures (D9, A3), every write gated on the account-to-employee link `0152` left for exactly this. An account with nothing linked reads one honest sentence per tab rather than an empty table pretending to be a normal one. **Handing a task's deliverable in as a file stays deferred**, per this note's own original scope: `link_entity_t` needs a `'task'` value first, and that is worth shaping from what people actually attach once the task module has been lived with for a while, not guessed at in the same session |
