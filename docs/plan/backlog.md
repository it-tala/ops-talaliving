...
| Q54 | **Is Friday short because the break is longer, or because people go home earlier?** | The owner says Friday is **7 hours**, which makes the week exactly 40. The schema has only one Friday lever — `friday_break_minutes` — and no Friday finishing time, so 7 hours forces a break of **120 minutes in produksi and 135 in kantor** (D288 applied this). That contradicts D270, where the owner's own Q44 answer was *Jumat istirahat 1,5 jam*: 90 minutes gives 7,5 and 7,75 hours, a week of 40,5 and 40,75. One of the two is wrong. If the break really is 1,5 hours and Friday is short because the workshop **closes earlier**, that needs a `friday_end_minutes` on the schedule row — a schema change, not a form entry, and the same shape as F91: a second answer the first shape cannot hold. Nothing is measured against it yet, so the cost of being wrong is one more version. **Default if unanswered**: the break, as applied |
