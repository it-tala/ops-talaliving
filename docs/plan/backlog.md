...
| B11 | ~~**Baris yang vendornya diputuskan di PO tidak bisa dibayar dari barisnya**~~ | `ops_acct.post_from_line` | **fixed** 2026-09-23 — `0133`: the vendor falls back to the order the line is on; a line on no order with no vendor is still refused `vendor_required` (F151) |
| B12 | ~~**Setiap PO draft di live bertuliskan *Changed since it was sent***~~ | `/procurement/po/[po]` | **fixed** 2026-09-23 — the ladder starts a draft at revision 1 with nothing sent (`0011`), the demo at 0/0, so only live showed it. The banner now needs an order that has gone out (F151) |
