...
function assetView(state: DemoState, a: Asset): AssetView {
  return {
    ...
    warranty_expired: !!a.warranty_until && a.warranty_until < officeToday()
...
export async function listAssets(
  ...
  if (!opts.include_gone && !opts.status) rows = rows.filter((a) => a.status !== "disposed" && a.status !== "lost");
...
/** Going (disposed, lost) needs a note and dates the end; coming back
 *  clears it. */
export async function setAssetStatus(
  ...
  const gone = status === "disposed" || status === "lost";
  if (gone && !note?.trim()) {
