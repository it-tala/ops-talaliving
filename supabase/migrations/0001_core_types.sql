...
--
-- **Into `ops_core`, and the schema is not a detail.** Every seam in this
-- system pins `search_path = ops_core, pg_temp`, which is what stops a caller
-- resolving one of our names to something of their own. A type whose operators
-- live outside that path is therefore invisible *inside the functions that use
-- it* — and Postgres does not raise for it. It falls back to comparing the
-- values as text, so `where email = p_email` silently becomes case-sensitive
-- and `bootstrap_admin('Budi@talaliving.com')` answers *no such user* about a
-- user who is right there. Measured, not assumed: with the extension in
-- `extensions` the lookup returns NULL, and in `ops_core` it returns the row.
--
-- It also keeps the extension out of `public`, which is where the default
-- `search_path` would have put it — and `public` belongs to the legacy system
-- sharing this database. The ladder touches nothing outside `ops_*`, and an
-- extension is not an exception to that.
--
-- Every use is schema-qualified as `ops_core.citext` for the same reason in
-- reverse: `ops_core` is not on the search_path when a migration runs.
create extension if not exists citext with schema ops_core;
